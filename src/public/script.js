...
function filterAndDisplayBuilds(
  builds,
  searchTerm = "",
  ...
  const weeklyBuild = builds.find(
    (build) => build.isWeekly && build.platform === "all"
  );
  ...
  const filteredBuilds = builds.filter((build) => {
    ...
    return (
      (!build.isWeekly || build.platform !== "all") &&
      matchesSearch &&
      matchesPlatform
    );
  });
...
function updateWeeklyBuildButton(weeklyBuild) {
  if (weeklyBuild) {
    weeklyBuildButton.onclick = () =>
      handleDownload(weeklyBuild.downloadUrl, weeklyBuild.id);
      ...
      (build) => `
        <div class="bg-dark-card rounded-xl p-6 card-shadow transition-all duration-300 ease-in-out hover:shadow-lg hover:shadow-primary/10 border border-gray-800/10">
            <div class="flex items-center gap-4 mb-4">
                <i class="fab fa-${getPlatformIcon(
                  build.platform
                )} text-3xl text-primary"></i>
                <div class="flex-1">
            ...
            <div class="flex justify-between items-center mb-4">
                <p class="text-gray-400">${new Date(
                  build.date
                ).toLocaleDateString()}</p>
                <button onclick="handleDownload('${build.downloadUrl}', '${
        build.id
      }')" 
